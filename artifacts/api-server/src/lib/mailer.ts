...
export async function sendChatbotAlert(params: {
  ...
  const safeCanditat = escapeHtml(params.candidat);
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:600px;background:#07020f;color:#e0e0e0;padding:24px;border-radius:8px;border:1px solid #8a4dff">
      ...
      <p><strong>Candidat :</strong> ${safeCanditat}</p>
      ${scoreHtml}
      ...
      <pre style="background:#1a1a2e;padding:12px;border-radius:4px;font-size:0.85em;color:#39f5ff;white-space:pre-wrap">${safeExcerpt}</pre>
